...
  // Hàm nén ảnh cho mobile (Đã tối ưu)
...
          
          // Chuyển canvas thành blob với chất lượng nén 0.8 
          canvas.toBlob(
            (blob) => {
              if (blob) {
                // Logic nén lại nếu vẫn quá lớn
                if (blob.size > 500000) { 
                  canvas.toBlob(
                    (compressedBlob) => {
...
      
      img.onerror = () => {
        reject(new Error('Không thể load ảnh. Ảnh có thể không hợp lệ.'));
      };
      
...
  // Hàm xử lý upload ảnh (Đã tối ưu bất đồng bộ)
...
            alert('Lỗi khi xử lý ảnh. Vui lòng thử lại: ' + error.message);
        }
    }
    
...
                  type="file"
                  accept="image/*"
                  // Đã thêm lại thuộc tính 'multiple'
                  multiple
                  // Giữ 'capture="environment"' để ưu tiên mở camera
                  capture="environment" 
...
                  <Camera size={20} />
                  {/* Chỉnh sửa mô tả để làm rõ cả 2 tùy chọn */}
...
                  <Plus size={16} />
                  {/* Chỉnh sửa mô tả */}
                  {uploading ? 'Đang xử lý...' : 'Thêm ảnh (Chụp hoặc Thư viện)'}
                </label>
              </div>
              
